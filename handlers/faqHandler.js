...
function buildEnhancedContext(config, prompt, guild, user) {
    ...
    } else if (lowerPrompt.includes('feature') || lowerPrompt.includes('bot') || lowerPrompt.includes('EMBER')) {
        contextPrompt += `**SPECIAL CONTEXT:** This is about bot features. Focus on EMBER Bot capabilities and relevant setup information.\n\n`;
    }
...
function createAdvancedResponseEmbed(aiResponse, config, guild, prompt, user) {
    ...
    } else if (lowerPrompt.includes('feature') || lowerPrompt.includes('EMBER') || lowerPrompt.includes('bot')) {
        embedColor = EMBED_COLORS.features;
        embedTitle = "EMBER Bot Features";
